...
form.addEventListener('submit',async event=>{
 ...
 try{
  const response=await fetch('/api/unlock-prompts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:form.email.value,website:form.website.value}),signal:AbortSignal.timeout(20000)});
  const data=await response.json();if(!response.ok)throw new Error(data.error||'Please try again.');
  if(typeof data.html!=='string')throw new Error('The library could not load. Please try again.');
  const remaining=document.querySelector('#remaining');remaining.innerHTML=data.html;remaining.hidden=false;
  document.querySelector('#gate').outerHTML='<section class="gate unlocked" aria-label="Signup complete"><div class="lock-mark" aria-hidden="true">✓</div><p class="eyebrow">YOU’RE IN</p><h2>All 100 are yours.</h2><p>Your signup is saved. Your full prompt library is open below.</p><a class="library-link" href="#remaining">Explore the prompts ↓</a></section>';
  remaining.focus();remaining.scrollIntoView({block:'start'});
 }catch(error){message.textContent=error.name==='TimeoutError'?'The connection took too long. Please try again.':error.message;button.disabled=false;button.textContent='Unlock all 100 prompts →';}
});
document.querySelector('#remaining').addEventListener('click',async event=>{
 ...
 const card=button.closest('.prompt-card');const value=card.querySelector('.prompt-label').textContent+' '+card.querySelector('.prompt-text').textContent;
